...
player_valuations_df = spark.read.format("csv").option("header", "true").load(f"{mount_point_bronze}/player_valuations/")

# Afficher un aperçu des données brutes
print("Données brutes de 'player_valuations':")
player_valuations_df.show(10)

# Nettoyage des données 'player_valuations'

# 1. Supprimer les colonnes inutiles (aucune identifiée pour l'instant)
columns_to_drop = []
player_valuations_cleaned = player_valuations_df.drop(*columns_to_drop)

# 2. Supprimer les lignes avec des valeurs nulles dans les colonnes critiques
columns_critical = ['player_id', 'market_value_in_eur', 'date']
player_valuations_cleaned = player_valuations_cleaned.dropna(subset=columns_critical)

# 3. Convertir les dates en format datetime
from pyspark.sql.functions import to_date
player_valuations_cleaned = player_valuations_cleaned.withColumn("date", to_date(player_valuations_cleaned["date"], "yyyy-MM-dd"))

# 4. Supprimer les doublons
player_valuations_cleaned = player_valuations_cleaned.dropDuplicates()

# 5. Trier les données par date et player_id
player_valuations_cleaned = player_valuations_cleaned.orderBy(["date", "player_id"])

# Afficher les données nettoyées
print("Données nettoyées et triées de 'player_valuations':")
player_valuations_cleaned.show(10)

# Sauvegarder les données nettoyées en un seul fichier dans Silver
temp_path = f"{mount_point_silver}/player_valuations_cleaned_temp/"
final_path = f"{mount_point_silver}/player_valuations_cleaned/player_valuations_cleaned.csv"

# Réduire à une seule partition et sauvegarder temporairement
player_valuations_cleaned.coalesce(1).write.format("csv").option("header", "true").mode("overwrite").save(temp_path)
...
display(dbutils.fs.ls(f"{mount_point_silver}/player_valuations_cleaned/"))
